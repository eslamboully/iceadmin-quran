...
@section('title') {{__('tripmodule::trip.vehicle')}}
@endsection
...
<section class="content-header">
    <h1> {{__('tripmodule::trip.vehicle')}} </h1>
...
<section class="content">
    ...
    <div class="box box-info">
        <div class="box-header with-border">
            <h3 class="box-title">{{__('tripmodule::trip.vehicle')}}</h3>
        </div>
        @if (count($errors) > 0) @foreach ($errors->all() as $item)
        <p class="alert alert-danger">{{$item}}</p>
        @endforeach @endif
        <!-- /.box-header -->
        <form class="form-horizontal" action="{{url('/admin-panel/vehicle')}}/{{ $vehicle->id }}" method="POST" enctype="multipart/form-data">
            {{ csrf_field() }}
            {{ method_field('PUT') }}
            ...
            <div class="box-body">
                        ...
                        <div class="tab-content">
                            ...
                            <div class="tab-pane @if($loop->first) active @endif" id="{{ $lang->display_lang }}">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-8">
                                        <input type="text" autocomplete="off" class="form-control" value="{{ ValueOf($vehicle, $lang, 'title') }}" name="{{$lang->lang}}[title]" @if($loop->first) required @endif>
                                    </div>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-8">
                                        <textarea id="editor{{ $lang->id }}" name="{{$lang->lang}}[description]" style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;">{{ ValueOf($vehicle, $lang, 'description') }}</textarea>
                                    </div>
                                </div>
                            </div>
                            @endforeach
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-4">
                        ...
                        @if($vehicle->photo)
                            <img src="{{asset('public/images/vehicle/' . $vehicle->photo)}}" height="70" width="100">
                        @else
            ...
            <div class="box-footer">
                <a href="{{url('/admin-panel/vehicle')}}" type="button" class="btn btn-default">{{__('tripmodule::trip.cancel')}} &nbsp; <i class="fa fa-remove" aria-hidden="true"></i> </a>
