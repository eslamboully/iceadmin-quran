...
//        dd($tripData , $trip_pics , $tripDestinations , $tripCategoriesId);
	...
	public function update(Request $request, $slug) {
		...
		$tripPic = $this->tripRepo->find($slug);
		$tripData = $request->except('_token', '_method', 'cover', 'photo', 'photos', 'de', 'en', 'es', 'ru', 'fr', 'it', 'destinations', 'categories');
		...
		if ($request->hasFile('cover')) {
			...
			$thumbnail_path = public_path() . '/images/trip/thumb/' . $tripPic->cover;
			$thumbnail_path2 = public_path() . '/images/trip/' . $tripPic->cover;
			File::delete([$thumbnail_path, $thumbnail_path2]);
		...
		$this->tripRepo->update($slug, $tripData, $trip_trans, $tripDestinations, $tripCategories);
	...
	public function dataTables() {
		...
		return DataTables::of($trips)
			...
			->addColumn('operations', function ($row) {
				...
				$edit_tag = '<a href="' . url("admin-panel/trip/" . $row->slug . "/edit") . '" type="button" class="btn btn-primary"><i class="fa fa-pencil" aria-hidden="true"></i></a>';
				$show_tag = '<a href="' . url("admin-panel/trip/" . $row->slug) . '" type="button" class="btn btn-success"><i class="fa fa-eye" aria-hidden="true"></i></a>';
				$prog_tag = '<a href="' . url("admin-panel/trip-program/" . $row->slug) . '" type="button" class="btn btn-warning"><i class="fa fa-map" aria-hidden="true"></i></a>';
